...
contents.id = function (articleName, formatId) {
    ...
    if (!formattedId.match(/^[a-z]+[a-z0-9\-_:\.]*$/)) {
        var err = 'Invalid ID: ' + formattedId + ' <-' + articleName;
        throw new Error(err);
    }
...
contents.link = function (guide, article) {
    ...
    article.className = 'contents-anchor-target';

    icon.className = 'fa fa-link';

    articleLink.href = '#' + articleId;
    articleLink.className = 'contents-anchor';
    articleLink.appendChild(icon);
    ...
    article.insertBefore(articleLink, article.childNodes[0]);
...
contents.indexOffset = function (elements) {
    ...
    while (i < j) {
        ...
        console.log('offset', offset, element);
